		...
		selectMenu = 'Selecionar personagem',
		back = 'Voltar',
		selectOp = 'Selecionar',
		deleteChar='Eliminar',
	...
	function rotateCamera()
		...
		SetCamCoord(cam,  nextCamLocation.x,  nextCamLocation.y,  nextCamLocation.z+0.7)
	...
	function saveCharData()
		...
		for i = 1,20 do
			...
			listfeatures[i]=featuresList[i-1].index
		...
		for i=1 ,12 do
			listOverlays[i]=overLays[i].index
	...
	AddEventHandler('loadCharacter', function(superString)
		...
		for i=1, 20 do
			...
			featuresList[i-1].index=tonumber(charData[i+5])
		...
		hairOptions.index=tonumber(charData[26])
		
		
		
		hairOptions.indexColor=tonumber(charData[27])
		hairOptions.indexColor2=tonumber(charData[28])
		
		for i=1,12 do
			overLays[i].index=tonumber(charData[28+i])
			overLays[i].opacity=tonumber(charData[40+i])
			overLays[i].color1=tonumber(charData[52+i])
...
Citizen.CreateThread(function()
	...
	--REDO EVERY THING DONT WORK BRUH ;:cc:c:c:c

	--WarMenu.OpenMenu('selectMenu')

	--[[

    while true do
	
		if WarMenu.IsMenuOpened('selectMenu') then
